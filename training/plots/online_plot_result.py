...
@File    ：online_plot_result.py
@Author  ：He Xing
@Date    ：2026/4/23 20:03 
"""
...
if __name__ == '__main__':
    ...
    root_folder = 'real_time_2level'
    solver_list = ['Euler', 'RK4']
    step_list = [20, 40]
    target_vars = ['p', 'q', 'vdc']
    skip_time = 0.5

    zoom_x_range = [1.95, 2.05]
    inset_pos = [0.35, 0.15, 0.4, 0.45]

    print("\n" + "=" * 70)
    print("Batch plotting for Euler and RK4")
    print("=" * 70)

    for solver in solver_list:

        data_folder = os.path.join(root_folder, solver)

        if not os.path.isdir(data_folder):
            print(f"[WARNING] Folder not found: {data_folder}, skip.")
            continue

        for step_us in step_list:

            print("\n" + "-" * 70)
            print(f"[CASE] Solver = {solver}, Step = {step_us} us")
            print("-" * 70)

            path_true = os.path.join(data_folder, f'Y_IGBT_{step_us}.mat')
            path_pred = os.path.join(data_folder, f'Y_ai_{step_us}.mat')
            path_swf = os.path.join(data_folder, f'Y_swf_{step_us}.mat')

            missing_files = [
                p for p in [path_true, path_pred, path_swf]
                if not os.path.exists(p)
            ]

            if missing_files:
                print("[WARNING] Missing files, skip this case:")
            ...
            print("\n" + "=" * 50)
            print(f" NRMSE Analysis | {solver} | {step_us} us | after {skip_time}s")
            print("=" * 50)
            ...
            for var in target_vars:
                save_name = os.path.join(
                    data_folder,
                    f'Result_HIL_{var}_Comparison_{solver}_{step_us}.svg'
                )
    ...
    print("\nAll batch plotting tasks completed.")